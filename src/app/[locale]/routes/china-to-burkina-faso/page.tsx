...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  const seo = getRouteSeo('china-to-burkina-faso', locale as Locale);
...
export default async function ChinaToBurkinaRoute({ params }: Props) {
  ...
  const breadcrumbs = [
    ...
    { name: seo.breadcrumb, url: `/${locale}/routes/china-to-burkina-faso` },
  ];
  ...
  return (
    <>
      ...
      />
      <RoutePage locale={locale} routeKey="china-to-burkina-faso" country={seo.country} capital={seo.capital} />
    </>
  );
}
