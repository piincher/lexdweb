...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  const seo = getRouteSeo('china-to-ghana', locale as Locale);
...
export default async function ChinaToGhanaRoute({ params }: Props) {
  ...
  const breadcrumbs = [
    ...
    { name: seo.breadcrumb, url: `/${locale}/routes/china-to-ghana` },
  ];
  ...
  return (
    <>
      ...
      />
      <RoutePage locale={locale} routeKey="china-to-ghana" country={seo.country} capital={seo.capital} />
    </>
  );
}
