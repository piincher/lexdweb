...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  const seo = getRouteSeo('china-to-guinea', locale as Locale);
...
export default async function ChinaToGuineaRoute({ params }: Props) {
  ...
  const breadcrumbs = [
    ...
    { name: seo.breadcrumb, url: `/${locale}/routes/china-to-guinea` },
  ];
  ...
  return (
    <>
      ...
      />
      <RoutePage locale={locale} routeKey="china-to-guinea" country={seo.country} capital={seo.capital} />
    </>
  );
}
