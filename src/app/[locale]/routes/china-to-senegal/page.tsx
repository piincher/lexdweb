...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  const seo = getRouteSeo('china-to-senegal', locale as Locale);
...
export default async function ChinaToSenegalRoute({ params }: Props) {
  ...
  const breadcrumbs = [
    ...
    { name: seo.breadcrumb, url: `/${locale}/routes/china-to-senegal` },
  ];
  ...
  return (
    <>
      ...
      />
      <RoutePage locale={locale} routeKey="china-to-senegal" country={seo.country} capital={seo.capital} />
    </>
  );
}
